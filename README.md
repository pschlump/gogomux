#GoGoRouter - A Fash HTTP Request Router

GoGoRouter is a high performance HTTP request router
...
Accoding to my dictionary, "Aggressivly Dynamic".
...
produciton.   This router is as fast (in my use case faster) than
HttpRouter.  It is also a near drop-in replacement for Gorilla Mux.

??? By "near drop-in replacement" I mean that I had to chagne the import and
add one call to router.CompileRoutes after switching to this router.

## Features

This is a drop in replacement for HttpRouter.  HttpRouter describes itself
as the **Best Perfromacne** router.  This router is on average faster.
[Benchmarks speak for themselves](http://www.gogomux.com/gogorouter/routing-benchmark.html).

This is also a quick replacement for Gorilla Mux.   A Perl script can do most
of the transation to switch from Gorrilla Mux to GoGoMux.

**Wild Card Parameters:**  Wild card patterns come in 3 forms.   `:name` will match any
single part of an URL after a slash and up to the next slash or the end of the URL.
`*name` will match to the end of the URL.   The are the two inexpensive pattern 
matches. `{name:RegularExpression}` will match a regular expression.   This is a
much more expensive match.   If you don't use `{name:...}` then you don't pay
any performance penalty.   Similarly if you are only routing static routes
it is faster than using `:name` and `*name` in your routes.


http://locust.io/ -- Performance testing
